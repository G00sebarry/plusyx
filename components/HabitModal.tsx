...
2 отжимания, 1 страница книги — это сохранит привычку живой. Главное правило: действие должно быть слишком простым, чтобы НЕ сделать его."
          ...
          <div className="flex flex-col gap-2">
            <div className="flex justify-between items-center px-1">
              ...
              {fileData && <button type="button" onClick={() => { setFileData(''); setCoverSettingsOpen(false); }} className="text-[9px] font-black text-red-500 uppercase">Удалить</button>}
            </div>
            <div className="flex items-center gap-3">
              <button type="button" onClick={() => fileInputRef.current?.click()} className="flex-1 tg-secondary-bg h-12 px-4 rounded-2xl text-[10px] font-black uppercase tracking-widest tg-text flex items-center justify-center gap-2 border border-dashed border-gray-400/20">{fileData ? '📸 Изменить' : '📷 Загрузить фото'}</button>
              {fileData && <button type="button" onClick={() => setCoverSettingsOpen(!coverSettingsOpen)} className={`w-12 h-12 rounded-2xl flex items-center justify-center text-lg transition-all ${coverSettingsOpen ? 'bg-blue-500/20 text-blue-500' : 'bg-black/5 tg-hint'}`}>⚙️</button>}
            </div>
            {fileData && (
